...
def _knowledge_base_path() -> Path:
    return Path(settings.KNOWLEDGE_BASE_DIR).expanduser().resolve()
...
def _relative_source(file_path: Path) -> str:
    ...
    try:
        return str(file_path.resolve().relative_to(kb_path))
    except ValueError:
...
def _load_single_file(file_path: Path) -> List[Document]:
    ...
    relative_source = _relative_source(file_path)
...
def load_and_index_documents() -> int:
    ...
    kb_dir = _knowledge_base_path()
    md_files = _discover_markdown_files(kb_dir)

    if not md_files:
        logger.warning("No markdown files found in %s, nothing to index.", kb_dir)
        return 0
